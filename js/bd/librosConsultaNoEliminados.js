...
import { validaLibro } from "../modelo/validaPlayera.js"
import { ALMACEN_LIBRO, Bd, INDICE_TITULO } from "./Bd.js"

export async function librosConsultaNoEliminados() {
 ...
 return bdConsulta(Bd, [ALMACEN_LIBRO],
  ...
  (transaccion, resolve) => {
   ...
   // Usa el índice INDICE_Titulo para recuperar los datos ordenados.
   ...
   consulta.onsuccess = () => {
    ...
    if (cursor === null) {
      ...
      *  cursor.value */
     const modelo = validaTitulo(cursor.value)
     if (modelo.LIB_AUTOR == "") {
...
exportaAHtml(librosConsultaNoEliminados)